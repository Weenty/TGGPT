...
#     BROWSER.add(cookies=storage_state["cookies"])

# set LocalStorage
    # BROWSER.setLocalStorage("local_storage", storage_state["local_storage"])
...
def check_and_wait_gpt_capacity(iterator):
    if is_logged_in():
        return False
    try:
        procces_login()
        ...
        time.sleep(iterator)
        PAGE.reload()
        check_and_wait_gpt_capacity(iterator+2)


def start_browser():
    PAGE.goto("https://chat.openai.com/chat")
    time.sleep(10)
    check_and_wait_gpt_capacity(1)
        

        
    # session_storage = os.environ["SESSION_STORAGE"]
    # BROWSER.add_init_script("""(storage => {
